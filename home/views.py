...
def add_to_wishlist(request,slug):
	...
	if Wishlist.objects.filter(slug = slug,username = username).exists():
		quantity,actual_price = calWishlist(slug)
		quantity = quantity+1
		total = actual_price*quantity

		Wishlist.objects.filter(slug = slug,username = username).update(
			quantity = quantity,
			total = total
			)
	else:# when user added product for first time in cart
		actual_price= calWishlist(slug)
...
class WishlistView(Base):
    def get(self,request):
        username = request.user.username
        self.views['my_wishlist'] = Wishlist.objects.filter(username = request.user.username)
